...
async def fetch_file_for_station(command_id):
    ...
    headers = {"accept": "*/*",  # CSV (API constraint)
               "apikey": API_TOKEN}
...
async def process_batch(batch, start_date, end_date, batch_index):
    ...
    async def create_one_station(station):
        ...
        if ok and cmd_id:
            ...
            logging.info(f"[Station {s_id}] Echec => pas de récupération.")
    ...
    for pass_index in range(1, max_passes+1):
        logging.info(f"=== Passe récup n°{pass_index}/{max_passes} ===")
        tasks = []
        stations_pending = set()
            ...
            async def do_fetch(station_id=s_id, cmd_id=c_id):
                ...
                if status == "done":
                    ...
                    enriched = {
                        ...
                        "rows": dict_data  # store as list of dictionaries
                    }
                    producer.send(TOPIC_NAME, key=station_id, value=enriched)
                    logging.info(f"[Station {station_id}] Données (liste de dictionnaires) publiées => commande={cmd_id}")
                    station_cmd_ids[station_id] = None

                elif status == "pending":
                    stations_pending.add(station_id)
                else:
                    # "error"
                    station_cmd_ids[station_id] = None
...
def main():
    ...
    if not batches:
        logging.error("Aucun batch trouvé.")
        return

    # Get dynamic dates for yesterday
    start_date, end_date = get_dynamic_dates_for_yesterday()
    logging.info(f"Lancement pour {len(batches)} batches: {start_date} => {end_date}")

    # Process each batch sequentially
    for i, batch in enumerate(batches):
        asyncio.run(process_batch(batch, start_date, end_date, i))
        if i < len(batches) - 1:
            logging.info(f"Pause 60s avant batch {i+2}.")
            asyncio.run(asyncio.sleep(60))

    logging.info("Fin de tous les batches.")
    producer.close()
